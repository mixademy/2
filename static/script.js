document.addEventListener('DOMContentLoaded', () => {
    ...
    const modelSelector = document.getElementById('model-selector') || { value: 'smart' }; // Biztonsági mentés ha hiányzik
    ...
    let attachedFileData = null; // Ide mentjük a backendről visszakapott fájltartalmat
    ...
    async function createNewChatFromBtn() {
        try {
            ...
            loadChats();
            messageInput.focus();
        } catch (e) { console.error("Hiba chat készítéskor:", e); }
    }
    ...
    // GLOBÁLIS FUNKCIÓK A 3 PÖTTYHÖZ
    window.toggleDropdown = function (btn) {
        document.querySelectorAll('.chat-dropdown.show').forEach(m => { if (m !== btn.nextElementSibling) m.classList.remove('show'); });
        btn.nextElementSibling.classList.toggle('show');
    };
    document.addEventListener('click', (e) => { if (!e.target.closest('.chat-actions')) document.querySelectorAll('.chat-dropdown.show').forEach(m => m.classList.remove('show')); });
    ...
    window.loadChatMessages = async (id, title) => {
        ...
        } catch (e) { console.error("Hiba az üzenetek betöltésekor:", e); }
    };
    ...
    if (fileUploadInput) {
        fileUploadInput.addEventListener('change', async function () {
            ...
            filePreviewLoading.style.display = 'inline-block'; // Töltés jelző
            ...
            try {
                ...
                if (data.success) {
                    attachedFileData = data; // Eltároljuk a Base64 képet vagy a kinyert PDF szöveget
                    filePreviewLoading.style.display = 'none';
                    filePreviewName.innerText = `${file.name} (Csatolva)`;
                } else {
                    alert("Hiba a fájl feldolgozása közben: " + data.error);
                    clearAttachment();
                }
            } catch (err) {
                console.error(err);
            ...
            // Töröljük az inputot, hogy ugyanazt a fájlt újra lehessen választani
    ...
    if (sendBtn) {
        sendBtn.addEventListener('click', async () => {
            try {
                ...
                // Csak akkor állunk meg, ha SE szöveg, SE fájl nincs
                ...
                // Vizuális megjelenítés a chatben
                let displayMsg = text;
                if (attachedFileData) {
                ...
                // Csatolás törlése a UI-ról küldés után
                const payloadFileData = attachedFileData;
                clearAttachment();
                ...
                const res = await fetch(`/api/chats/${currentChatId}/message`, {
                    ...
                    body: JSON.stringify({
                        ...
                        file: payloadFileData // Átküldjük a fájlt is!
                    }),
                ...
                if (e.name === 'AbortError') console.log("Kérés megszakítva.");
                else {
                    console.error("KÜLDÉSI HIBA:", e);
                    const typingMsg = document.querySelector('.message.assistant .fa-spin');
                    if (typingMsg) typingMsg.parentElement.innerHTML = 'Hálózati hiba történt.';
                }
            } finally {
                resetSendButton();
                scrollToBottom();
